...
Construiești un dashboard care vizualizează date bursiere simulate pentru cinci companii. Utilizatorul selectează o companie dintr-un `ComboBox`, iar cele trei grafice se actualizează automat: evoluția prețului de închidere, volumul tranzacționat și graficul Candlestick cu datele OHLC complete.

Datele sunt hardcodate într-un `FakeDatabase` static — nu există bază de date și nu există repository în acest laborator. Scopul este exclusiv familiarizarea cu Chart Control.

### Modelul și datele

**Clasa `Companie`:**

* `string Simbol` — ex. `"TLV"`, `"SNP"`
* `string Nume` — ex. `"Banca Transilvania"`
* `string Sector` — ex. `"Financiar"`, `"Energie"`

**Clasa `ZiBursiera`:**

* `string Simbol` — simbolul companiei căreia îi aparține
* `DateTime Data`
* `decimal Open`
* `decimal High`
* `decimal Low`
...
Declară `static List<Companie> Companii` cu cel puțin 3 companii și `static List<ZiBursiera> Zile` cu cel puțin 14 de zile per companie, cu date simulate rezonabile. Un exemplu se găsește mai jos.

### Structura ferestrei principale

Pe `Form1` adaugă:

* Un `Label` și un `ComboBox` (`cmbCompanie`) deasupra, pentru selectarea companiei
* Un `TabControl` (`tabControl`) cu trei taburi: `"Preț"`, `"Volum"`, `"Candlestick"`
* Câte un control `Chart` în fiecare tab: `chartPret`, `chartVolum`, `chartCandlestick`
* Fiecare `Chart` cu `Dock = Fill`

Populează `cmbCompanie` în `Form1_Load` cu obiectele `Companie` din `FakeDatabase`. Setează `DisplayMember = "Nume"` pe `ComboBox` pentru a afișa numele companiei. Selectează prima companie implicit.

### Line chart: evoluția prețului

Implementează metoda `IncarcaChartPret(Companie companie)`.

Filtrează `FakeDatabase.Zile` după simbolul companiei, ordonat crescător după `Data`. Configurează `chartPret` cu:

* O singură `Series` de tip `Line`, denumită `"Preț închidere"`
* Valorile X: data formatată `"dd.MM"`, valorile Y: prețul `Close`
* `ChartArea` cu titlu, axe formatate și `GridLines` vizibile
* Culoarea liniei și grosimea configurate vizual

### Bar chart: volumul tranzacționat

Implementează metoda `IncarcaChartVolum(Companie companie)`.

Configurează `chartVolum` cu:

* O singură `Series` de tip `Bar` sau `Column`, denumită `"Volum"`
* Valorile X: data, valorile Y: volumul
* Axa Y formatată pentru numere mari (ex. `"#,##0"`)
* Culoarea barelor configurată

### Candlestick chart

Implementează metoda `IncarcaChartCandlestick(Companie companie)`.

Configurează `chartCandlestick` cu:

* O singură `Series` de tip `Candlestick`
* Fiecare punct adăugat cu `AddXY(data, high, low, open, close)` — atenție la ordinea parametrilor
* Culoare pentru zile pozitive (Close > Open) și negativă (Close < Open)
* Axa X cu datele formatate, axa Y cu prețurile

### Actualizarea la schimbarea companiei

Abonează-te la evenimentul `SelectedIndexChanged` al `cmbCompanie`. La schimbarea selecției, apelează toate cele trei metode de încărcare cu noua companie selectată. Toate graficele se actualizează simultan.
...
```
GET https://ase.lucianvilcea.ro/programarea-aplicatiilor-windows/seminar-8-chart-control/exercitiu.md?ask=<question>
```
